...
class PlanningGraph():
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return len(set(node_a1.action.effect_add).intersection(node_a2.action.effect_rem)) >= 1 or \
               len(set(node_a1.action.effect_rem).intersection(node_a2.action.effect_add)) >= 1
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return len([pre for pre in node_a1.action.precond_pos if pre in node_a2.action.effect_rem]) >= 1 or \
               len([pre for pre in node_a1.action.precond_neg if pre in node_a2.action.effect_add]) >= 1 or \
               len([pre for pre in node_a2.action.precond_pos if pre in node_a1.action.effect_rem]) >= 1 or \
               len([pre for pre in node_a2.action.precond_neg if pre in node_a1.action.effect_add]) >= 1
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return len([(p1, p2) for p1 in node_a1.parents for p2 in node_a2.parents if p1.is_mutex(p2) or p2.is_mutex(p1)]) >= 1
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for action_1 in node_s1.parents:
            for action_2 in node_s2.parents:
                if not(action_1.is_mutex(action_2)) or not(action_2.is_mutex(action_1)):
                    return False
        return True
